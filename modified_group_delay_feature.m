function [grp_phase, cep, ts] = modified_group_delay_feature(file_name, rho, gamma, num_coeff, frame_shift)
    ...
    % Francis Tom used the following: Hamming window of 50ms and shift of 25ms,
    % 2048 FFT points used. Truncated or padded to a length of 256 along the
    % time axis. rho and gamma ? Resized to 512 x 256

    %Group delay does not have rho and gamma 
    ...
    grp_phase = (real(x_spec).*real(y_spec) + imag(y_spec) .* imag(x_spec)) ./(exp((temp_x_spec)).^2);
