...
NAME = "resnet_advesarial_btas2016_2016-{}".format(int(time.time()))
...
#Training Data
pickle_in = open(os.path.join(DATADIR, "specTrain_win400_hop160_5s.pickle"), "rb")
specTrain_2017 = pickle.load(pickle_in)
pickle_in.close()

#Development Data
pickle_in = open(os.path.join(DATADIR, "specDev_win400_hop160_5s.pickle"), "rb")
specDev_2017 = pickle.load(pickle_in)
pickle_in.close()

#Evaluation Data
pickle_in = open(os.path.join(DATADIR, "specEval_win400_hop160_5s.pickle"), "rb")
...
model_combined, model_class = resnet.DomainResnetBuilder.build_resnet_18((1, 257, 501), 2, hp_lambda)
...
#categorical_focal_loss(alpha=0.39, gamma=10)

model_combined.compile(optimizer=adam, loss=[masked_loss_function, "categorical_crossentropy"], metrics=["accuracy"])
...
model_combined.fit(
        ...
        batch_size=8,
        verbose=2,
        validation_data= (X_test_2017, [y_test_2017, y_test_domain]), callbacks=[tensorboard, developmentSetEval(y_test_2017_saved)])
...
final = []
f = open(os.path.join(DATADIR, "predictions_btasTrain_2017eval_advesarial_2.txt"), "w+")
for pred in predictions:
    f.write(str(pred[1]) + "\n")
    if (pred[1] > 0):
